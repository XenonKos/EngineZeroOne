...
bool    ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    // Create the root signature
    {
        ...
        HRESULT hr = bd->pd3dDevice->CreateRootSignature(0, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&bd->pRootSignature));
        if (hr != S_OK) {
            HRESULT removedReson = bd->pd3dDevice->GetDeviceRemovedReason();
            return false;
        }
        blob->Release();
    }
